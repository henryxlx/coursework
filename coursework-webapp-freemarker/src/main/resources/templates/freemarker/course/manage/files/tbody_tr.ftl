    ...
    <td>
        ...
        <#else>
            ...
            <#if ['courselesson', 'coursematerial']?seq_contains(uploadFile.targetType) >
                <a href="${ctx}/course/${course.id}/manage/file/${uploadFile.id}" target="_blank" rel="tooltip" title data-original-title="{{ uploadFile.filename }}"{{ uploadFile.filename||sub_text(30) }}</a>
                <#if uploadFile.type == 'ppt' && ( !uploadFile.metas2.length||default(null) ) && uploadFile.targetType == 'courselesson'>
                    <br><a href="javascript:;" data-url="${ctx}/course/${course.id}/manage/file/${uploadFile.id}/convert" class="convert-file-btn text-danger">重新转换</a>
                </#if>
            <#else>
                <a rel="tooltip" title data-original-title="{{ uploadFile.filename }}"  >{{ uploadFile.filename||sub_text(30) }}</a>
            </#if>
        ...
        <#if setting('developer.debug')?? && uploadFile.convertParams.convertor||default(null)>
        <a href="javascript:;" data-url="${ctx}/course/${course.id}/manage/file/${uploadFile.id}/convert" class="convert-file-btn text-danger">重新转换(调试模式)</a>
        </#if>
        ...
        <#if uploadFile.type == "video" && uploadFile.metas2||default(null)>
